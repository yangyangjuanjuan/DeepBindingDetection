...
3) positive cases: binding sites. Under "bindingsites" folder. As an example, file 'loadBindingSite/binding_sites_CDX2'
looks like:
...
def grab_highscore_xmers(path,TF,dictionary):
    '''
    :param path: path to binding site
    :param TF: TF which want to load high score negative seqs
    :param dictionary: dictionary
    :return: a list seqs[]
    '''
    sentences = []
    ...
    with open(TF, 'r') as f:
        for line in f.readlines():
            sentences.append(list(line.strip().lower())[1:-1])
    os.chdir(currdir)
    seqs = [None] * len(sentences)
    for idx, ss in enumerate(sentences):
        seqs[idx] = [dictionary[w] if w in dictionary else 1 for w in ss]
...
def main():
    ...
    for folder in os.listdir("./bindingsites"):
        ...
        train_x_neg_highscore = grab_highscore_xmers(path+'/BSPHighPWMScoreNonBinding',folder,dictionary)
        
        train_x_neg=random.sample(train_x_neg,10000-10000/ratio)
        if len(train_x_neg_highscore)>10000/ratio:
            train_x_neg_highscore=random.sample(train_x_neg_highscore,10000/ratio)
            train_x_neg=random.sample(train_x_neg,10000-10000/ratio)
        else:
            train_x_neg=random.sample(train_x_neg,(ratio-1)*len(train_x_neg_highscore))
        train_x = train_x_pos + train_x_neg + train_x_neg_highscore
        train_y = [1] * len(train_x_pos) + [0] * len(train_x_neg)+ [0] * len(train_x_neg_highscore)
        ...
        f = open(os.getcwd()+path+'/bindingsitespkl_highscorenegative/'+folder+'_bindingsites.pkl', 'wb')
